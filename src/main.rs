...
impl State {
    ...
    fn game(&mut self, ctx: &mut BTerm) {
        ctx.cls_bg(NAVY);
    ...
    fn game_over(&mut self, ctx: &mut BTerm) {
        ctx.cls();
        ctx.print_centered(8, "You died, game over");
        ctx.print_centered(10, &format!("Final score: {}", self.score));
